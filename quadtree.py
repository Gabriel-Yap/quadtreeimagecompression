...
def build_quadtree_recursive(qt, node):
    ...
    """
      # base case check if node represents a single pixel
    if node.ul_x == node.dr_x and node.ul_y == node.dr_y:
        # assign the pixel color from the image
        node.color = qt.image[node.ul_y, node.ul_x]
        return node
    
    # if not single pixel, create 4 children for each quadrant
...
def quadtree_to_image_recursive(qt, node, draw_box):
    ...
    """
    # 1) First check if current node has color, if yes, draw_color
    # 2) if no color, check if child exist, if yes, recurse
...
def compress_quadtree_recursive(qt, node, threshold):
    ...
    """
     # base case: leaf node 
    if node.nw is None and node.ne is None and node.sw is None and node.se is None:
        return
    
    # Recursively compress all children first (post-order traversal)
    ...
    # combine children's colors
    combined_color = combine_colors(node, threshold)
    
    # If colors can be combined:
    if combined_color is not None:
        node.color = combined_color # 1. Set the node's color to the combined color
        node.nw = None              # 2. Set children to None
        node.ne = None
        node.sw = None
        node.se = None
...
def count_nodes(node):
    ...
    if node is None:
        return 0
    # Count current node + all nodes in each subtree
